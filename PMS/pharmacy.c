...
struct node* createadmin(struct node *head,int data, char productname[25], float price)
{
...
void displayList(struct node *head)
{
    ...
    else
    {
        ...
        printf("\t\t\t\t\t\t\t\t\tPRODUCTS:\n\n");
        if(temp1->quantity==0)
        	printf("\t\t\t\t\t\t\tS.N\tProduct\t\t\tPrice\n\n");
        else
        	{
        		printf("\t\t\t\t\t\t\tS.N\tProduct\t\tQuantity\tPrice\n\n");
			}
        while(temp1!=NULL)
        {
            if(temp1->quantity==0)
                printf("\t\t\t\t\t\t\t%d.\t%s\t\t%0.2f\n",temp1->data,temp1->productname,temp1->price);
            else
            {
                printf("\t\t\t\t\t\t\t%d.\t%s\t%d\t\t%0.2f\n",temp1->data,temp1->productname,temp1->quantity,temp1->price);
            }
...
int deleteadmin()
{
    printf("\n\t\t\t\t\tEnter serial no. of the product which is to be deleted: ");
    int num;
    scanf("%d",&num);
    ...
    while(1)
    {
        ...
        switch (opt)
        {
            ...
            case 2:
                ...
                printf("\n\t\t\t\t\t\t\tEnter serial no. of the product item: ");
                int num,flag = 0;
                char name[50];
                float price;
                scanf("%d",&num);
                ...
                while(temp!=NULL)
                {
                    if(temp->data==num)
                    {
                        printf("\n\t\t\t\t\t\tProduct with given serial number already exists!!\n\n");
                        printf("\t\t\t\t\t   ----------------------------------------------\n");
                        flag = 1;
                        break;
                    }
            ...
            case 3:
                if(deleteadmin())
                {
                    printf("\n\t\t\t\t\t\t### Updated list of products menu ###\n");
                    displayList(heada);
                }
                else
                    printf("\n\t\t\t\t\t\tProduct with given serial number doesn't exist!\n\n");
                    printf("\t\t\t\t\t   ----------------------------------------------\n");
    ...
    while(1)
    {
        ...
        switch (opt)
        {
            ...
            case 3:
                if(deletecustomer())
                {
                    printf("\n\t\t\t\t\t\t### Updated list of your ordered product items ###\n");
                    displayList(headc);
                }
                else
                    printf("\n\t\t\t\t\t\tProduct with given serial number doesn't exist!!\n");
...
int main()
{
    heada = createadmin(heada,1,"Paracetamol",100);
    heada = createadmin(heada,2,"Diegene(LQ)",200);
    heada = createadmin(heada,3,"Meftal(SG)",150);
    heada = createadmin(heada,4,"Omeprazole",180);
    heada = createadmin(heada,5,"Amoxicillin",80);
